...
contract MemeFactoryTest is Test {
    ...
    function testMintMeme() public {
        // Deploy meme
        vm.prank(creator);
        address token = factory.deployMeme("MOON", 1000000, 1000, 1e15);
        
        // Debug: Check the perMint value in the deployed contract
        uint256 contractPerMint = Meme(token).perMint();
        console.log("Contract perMint:", contractPerMint);
        console.log("Expected perMint:", uint256(1000 * 1e18));
        ...
        // Mint tokens
        vm.prank(buyer1);
        bool success = factory.mintMeme{value: cost}(token);
        assertTrue(success);
        
        // Debug: Check actual balance
        uint256 actualBalance = IERC20(token).balanceOf(buyer1);
        console.log("Actual balance:", actualBalance);
        console.log("Expected balance:", uint256(1000 * 1e18));
        
        // Check token balance (should be perMint * 1e18)
        assertEq(actualBalance, contractPerMint);
        
        // Check minted amount updated
        MemeFactory.MemeInfo memory info = factory.getMemeInfo(token);
        assertEq(info.minted, 1000);
    }
    
    function testFeeDistribution() public {
        // Deploy meme
        vm.prank(creator);
        address token = factory.deployMeme("FEE", 1000000, 1000, 1e15);
        
        uint256 cost = 1e15 * 1000; // 1 ETH total
        uint256 expectedPlatformFee = cost * 100 / 10000; // 1% = 0.01 ETH
        uint256 expectedCreatorFee = cost - expectedPlatformFee; // 99% = 0.99 ETH
        ...
        assertEq(ownerBalanceAfter - ownerBalanceBefore, expectedPlatformFee);
        assertEq(creatorBalanceAfter - creatorBalanceBefore, expectedCreatorFee);
    }
    
    function testMintMemeEmitsEvent() public {
        vm.prank(creator);
        address token = factory.deployMeme("EVENT", 1000000, 1000, 1e15);
        ...
        emit MemeMinted(token, buyer1, 1000 * 1e18, cost, platformFee, creatorFee);
    ...
    function testRefundExcessPayment() public {
        vm.prank(creator);
        address token = factory.deployMeme("REFUND", 1000000, 1000, 1e15);
        
        uint256 cost = 1e15 * 1000; // 1 ETH
        uint256 excessPayment = 0.5 ether;
        uint256 totalPayment = cost + excessPayment;
        
        uint256 buyer1BalanceBefore = buyer1.balance;
        
        vm.prank(buyer1);
        factory.mintMeme{value: totalPayment}(token);
        
        uint256 buyer1BalanceAfter = buyer1.balance;
        
        // Should refund excess payment
        assertEq(buyer1BalanceBefore - buyer1BalanceAfter, cost);
    }
    ...
    function testCannotExceedMaxSupply() public {
        vm.prank(creator);
        address token = factory.deployMeme("LIMITED", 1000, 1000, 1e15); // Only 1000 total, 1000 per mint
        ...
        // Second mint should fail (would exceed max supply)
        vm.prank(buyer2);
    ...
    function testMultipleMints() public {
        vm.prank(creator);
        address token = factory.deployMeme("MULTI", 10000, 1000, 1e15);
        ...
        assertEq(IERC20(token).balanceOf(buyer2), 1000 * 1e18);
        assertEq(IERC20(token).totalSupply(), 2000 * 1e18);
        
        // Check minted amount
        MemeFactory.MemeInfo memory info = factory.getMemeInfo(token);
        assertEq(info.minted, 2000);
    }
